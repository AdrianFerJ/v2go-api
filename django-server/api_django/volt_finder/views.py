        ...
        if 'poi_lat' in data and 'poi_lng' in data:
            ...
            if 'start_datetime' in data and 'end_datetime' in data:
                ...
                cs_avail = ChargingStation.objects.filter(
                    eventcs__status=constants.AVAILABLE, eventcs__startDateTime__range=(
                ...
                if len(cs_near_poi) > 10:
                    ...
                    serializer = CSwithDistanceSerializer(
                        cs_near_poi[0:10], many=True)

                    return Response(serializer.data)
                else:
                    serializer = CSwithDistanceSerializer(
                        cs_near_poi, many=True)
                    return Response(serializer.data)
            else:
                # Get all CS within max_dist (in meters) of POI, and annotate
                # distance to poi
