...
class TestEventCS(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cs_event_1 = EventCS.objects.create(
            startDateTime=dt.strptime(
                '2019-09-25 12:00:00', '%Y-%m-%d %H:%M:%S'),
            endDateTime=dt.strptime(
                '2019-09-25 12:30:00', '%Y-%m-%d %H:%M:%S'),
            cs=cls.cs_t1,
        ...
        cls.cs_event_2 = EventCS.objects.create(
            startDateTime=dt.strptime(
                '2019-09-25 15:00:00', '%Y-%m-%d %H:%M:%S'),
            endDateTime=dt.strptime(
                '2019-09-25 15:30:00', '%Y-%m-%d %H:%M:%S'),
            cs=cls.cs_t1,
        ...
        cls.cs_event_3 = EventCS.objects.create(
            startDateTime=dt.strptime(
                '2019-09-27 12:00:00', '%Y-%m-%d %H:%M:%S'),
            endDateTime=dt.strptime(
                '2019-09-27 12:30:00', '%Y-%m-%d %H:%M:%S'),
            cs=cls.cs_t1,
            status=constants.AVAILABLE
        )
        ...
        cls.cs_event_4 = EventCS.objects.create(
            startDateTime=dt.strptime(
                '2019-09-28 12:00:00', '%Y-%m-%d %H:%M:%S'),
            endDateTime=dt.strptime(
                '2019-09-28 12:30:00', '%Y-%m-%d %H:%M:%S'),
            cs=cls.cs_t1,
            ev_event_id=1,
            status=constants.RESERVED,
        )
    ...
    def setUp(self):
        ...
        self.client.login(username=self.cs_host.username,
...
class TestEventEV(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cs_event_4 = EventCS.objects.create(
            startDateTime=dt.strptime(
                '2019-09-29 12:00:00', '%Y-%m-%d %H:%M:%S'),
            endDateTime=dt.strptime(
                '2019-09-29 12:30:00', '%Y-%m-%d %H:%M:%S'),
            cs=cls.cs_t1,
            status=constants.AVAILABLE
        )
        ...
        cls.cs_event_5 = EventCS.objects.create(
            startDateTime=dt.strptime(
                '2019-09-30 12:00:00', '%Y-%m-%d %H:%M:%S'),
            endDateTime=dt.strptime(
                '2019-09-30 13:00:00', '%Y-%m-%d %H:%M:%S'),
            cs=cls.cs_t1,
            status=constants.AVAILABLE
        )
    ...
    def setUp(self):
        self.client = APIClient()
        self.client.login(username=self.ev_driver.username,
                          password=constants.PASSWORD)
    ...
    def test_driver_can_reserve_available_charging_station(self):
        ...
        self.assertEqual(response.data['event_cs'],
                         cs_event_to_ordered_dict(self.cs_event_1))
    ...
    def test_driver_can_view_completed_events_list(self):
        ...
        self.assertEqual(response.data[0]['event_cs'],
                         cs_event_to_ordered_dict(self.cs_event_3))
    ...
    def test_driver_can_cancel_reservation(self):
        ...
        self.assertEqual(reserved.data['event_cs'],
                         cs_event_to_ordered_dict(self.cs_event_1))
