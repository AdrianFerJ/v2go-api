...
class ChargingStation(models.Model):
    ...
    calendar = models.OneToOneField(
        Calendar, null=True, on_delete=models.CASCADE)

    charge_level = models.CharField(
        max_length=32,
        choices=CHARGE_LEVEL,
        default=LEVEL_2)
    ...
    def save(self, **kwargs):
        ...
        if not self.geo_location:
            try:
                ...
                # TODO: add test (if lat-lng not valid, etc) and add Log if
                # geolocation not created
                pass

        if not self.calendar:
            self.calendar = Calendar.objects.create(
                name=self.name, slug=self.nk)
...
class ElectricVehicle(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.calendar:
            self.calendar = Calendar.objects.create(
                name=self.model, slug=self.nk)

        super().save(*args, **kwargs)

    def __str__(self):
        return "%s, %s, %s, %s" % (
            self.manufacturer, self.model, self.nickname, self.updated)
